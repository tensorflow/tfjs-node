...
export function decodeImage(
  ...
  const image = fs.readFileSync(path, {encoding: 'hex'});
  ...
  switch (imageType) {
    case JPEG:
      return backend.decodeJpeg(
          ...
          acceptableFraction, dctMethod);
    case PNG:
      return backend.decodePng(uint8array, channels);
    case GIF:
      return backend.decodeGif(uint8array);
  }
  return null;
}
...
function getImageType(buf: Buffer): string {
  ...
  // Ccode of classifying file type:
