...
class SavedModel {
  ...
  run(inputTensor: Tensor<Rank>): Tensor<Rank> {
    return this.backend.runSession(this.id, inputTensor);
  }
}
...
export class NodeJSKernelBackend extends KernelBackend {
  ...
  runSession(sessionId: number, x: Tensor): Tensor {
    // const inputArgs = [scalar(path, 'string')];
    const outputMetadata =
        this.binding.runSession(sessionId, this.getInputTensorIds([x])[0]);
