...
import {TensorHandle, Context} from './tfnodejs';
// TODO - load this from elsehwere?
...
export class NodeJSKernelBackend implements KernelBackend {
  ...
  constructor() {
    this.context = new binding.Context();
  }
  ...
  matMul(a: Tensor2D, b: Tensor2D, transposeA: boolean, transposeB: boolean):
      Tensor2D {
    const opAttrs = [
      {name: 'transpose_a', type: binding.TF_ATTR_BOOL, value: transposeA},
      {name: 'transpose_b', type: binding.TF_ATTR_BOOL, value: transposeB},
  ...
  pad<T extends Tensor<Rank>>(
    ...
    const opAttrs = [
      {name: 'T', type: binding.TF_ATTR_TYPE, value: binding.TF_FLOAT},
      {name: 'Tpaddings', type: binding.TF_ATTR_TYPE, value: binding.TF_INT32}
    ];
  ...
  register(dataId: object, shape: number[], dtype: 'float32'|'int32'|'bool'):
    ...
    switch (dtype) {
      case 'float32':
        tfDtype = binding.TF_FLOAT;
        break;
      case 'int32':
        tfDtype = binding.TF_INT32;
        break;
      case 'bool':
        tfDtype = binding.TF_BOOL;
        break;
      default:
        console.log('unknown');
    }

    this.tensorMap.set(dataId, new binding.TensorHandle(shape, tfDtype));
  }
